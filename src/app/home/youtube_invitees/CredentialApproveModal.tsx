...
const ApproveCredentialModal: React.FC<{
  ...
  useEffect(() => {
    if (open) {
      setCheckData({
        ...{ gmail: false, duration: false, plan: false, auth: false },
      });
    }
  }, [open]);
  ...
  return (
    <Modal
      title={`Approve  Credential (${selectedPlatform?.email})`}
    ...
    >
      <div className=" h-60 mt-10 flex flex-col gap-y-7">
        ...
        <Checkbox onChange={(e) => onChange("plan", e.target.checked)}>
          I confirmed that ( {plan?.pricingName} ) is selected and paid for
        </Checkbox>
