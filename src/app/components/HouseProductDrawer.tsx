...
const HouseProductDrawer: React.FC<DrawerProps> = ({ product, onClose, open }) => {
    ...
    const onToggleService = (platform: any) => {
        ...
        if (index === -1) {
            if ((selectedPlatforms.length + 1) > (product?.total_selection_count ?? 0)) {
    ...
    const getOptions = () => {
        const price = product?.plan_price ?? 0;
        const platformName = (platforms ?? [])[0]?.title
                ...
                {!isNotAvailable && !isComplete && product && (
                    <div className="flex flex-col items-start py-6">
                        ...
                        <div className="mt-4 mb-1 px-3 flex justify-between items-center w-full">
                            <span className="text-lg">{product?.plan_name}</span>
                ...
                {product?.total_selection}
                        ...
                        <div className="mt-3 flex flex-col px-3 w-full gap-y-2">
                            {platforms!.map((pl) => (
                                ...
                                >
                                    <div className="flex items-center gap-x-3">
                                        <img src={pl.extra_icon} className="w-6 h-6" />
                                    ...
                                    <div>
                                        <Icon
                                            icon={
                                                selectedPlatforms.includes(pl.title)
...
export default HouseProductDrawer;
